...
import { CircularProgressOffAC, CircularProgressOnAC } from './state/circularProgressReducer'
...
export function TodolistWithRedux({ todolist }: PropsType) {
	const isLoading = useSelector<AppRootStateType, boolean>(s => s.circularProgress.isProgress)
	...
	const dispatchWithLoading = (action: TasksActionsType) => {
		dispatch(CircularProgressOnAC())
		dispatch(action)
		setTimeout(() => dispatch(CircularProgressOffAC()), 20000)
	}
